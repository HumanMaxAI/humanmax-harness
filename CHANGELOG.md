...
- `humanmax add tool` writes a `docs/tools/<id>.md` stub in the same change set as the declaration, source, test, and registry entry. The stub points at `.humanmax/`, requires the action gateway, and does not claim certification or production enforcement.
