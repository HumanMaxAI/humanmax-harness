...
function renderFiles(
  ...
  const runtimeDep = fileDep(destination, "runtime-harness");
  const contractsDep = fileDep(destination, "contracts");
  ...
  const files: GenerateFile[] = [
    file(
      "package.json",
      "mergeable",
      packageJson(name, runtimeDep, contractsDep, fileDep(destination, "cli")),
    ),
  ...
  return `${JSON.stringify(
    {
      ...
      dependencies: {
        ...
        "@humanmax/runtime-harness": runtimeDep,
      },
      devDependencies: {
        "@humanmax/cli": cliDep,
      },
...
This project is local-first. The fixture run needs no provider credentials and no HumanMax account. Passing these tests is not production enforcement or certification.
...
jobs:
  harness:
    ...
    steps:
      ...
      - run: npm ci
      - run: npm test
      - run: npx humanmax generate --check --format json
      - run: npx humanmax check --format json
`;
}
